...
// innerHTML anywhere in src/ -- the drawings are SVG built element by element
// with `svgEl`, and their labels are text nodes. A page that builds its rows
// with `el` cannot have an injection bug through a cluster's name or a backup
// tool's error message, which is the one place cluster data is genuinely
// hostile.
